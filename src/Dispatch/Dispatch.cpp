...
DispatchOperationGroupContainer::DispatchOperationGroupContainer(DispatchOperationGroupP group)
:DispatchOperation(nullptr)
,_group(group)
{}

DispatchOperationState DispatchOperationGroupContainer::GetState()
{
    if (_group->IsExecuting()) {
        return DispatchOperationState_Running;
    }
    if (_group->IsCancelled()) {
        return DispatchOperationState_Cancelled;
    }
    return DispatchOperationState_Finished;
}

bool DispatchOperationGroupContainer::IsCancelled()
{
    return _group->IsCancelled();
}

void DispatchOperationGroupContainer::Cancel()
{
    _group->Cancel();
}
...
DispatchQueueP Dispatch::CreateSpecialQueueWithId(const std::string &queueId)
{
    assert(_backgroundQueuesSpecial.count(queueId) == 0);
    auto queue = DispatchBackgroundQueueP(new DispatchBackgroundQueue(this));
    _backgroundQueuesSpecial[queueId] = queue;
    return queue;
}

DispatchQueueP Dispatch::GetSpecialQueueWithId(const std::string &queueId)
{
    auto f = _backgroundQueuesSpecial.find(queueId);
    if (f == _backgroundQueuesSpecial.end()) {
        return nullptr;
    }
    return f->second;
}
